    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    login(request, user)
                    return redirect(reverse_lazy('lms_pro:home'))
    else:
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if form.cleaned_data['is_instructor'] is True:
                permissions = Permission.objects.get(name='Can add course')
                instructors_group = Group.objects.get(name='instructors')
                user.groups.add(instructors_group)
                user.user_permissions.add(permissions)
            else:
                permissions = Permission.objects.get(name='Can view course')
